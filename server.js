...
const fetch = require('node-fetch'); // Agar node-fetch installed na ho toh package.json mein add kar lena ya native fetch use karein
const app = express();
...
// Telegram Bot par message bhejne ka code
app.post('/send-message', async (req, res) => {
    ...
    if (!BOT_TOKEN || !CHAT_ID) {
        return res.status(500).send({ success: false, error: "Bot token or Chat ID missing" });
    }
